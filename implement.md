...
- When `data/train.parquet` and `data/test.parquet` are mounted on the current host, treat them as the source of truth and do not waste time trying to rediscover the basic schema from scratch.
- If the current host is missing `data/`, restore or mount it before running data-dependent training, evaluation, or submission commands, and log the blocker in `documentation.md`.
- Use `.venv` as the default Python environment for all commands in this repo.
- Hardware available for this repo:
  - `NVIDIA RTX A5000` (`24564 MiB` VRAM)
  - `251 GiB` RAM visible to the OS
  - `AMD EPYC 7713P` (`128` logical CPUs)
  - `WD_BLACK SN850X` storage
- LightGBM runtime defaults on this host cap auto `num_threads` at `64` and enable `force_col_wise` on many-core machines; expect better CPU throughput than the old workstation, but do not plan on using all `128` logical CPUs in one LightGBM fit.
...
- for already-clipped warm subsets, inspect raw `error_sum` / `denom_sum` deltas before calling a move neutral or promising,
- do not promote a candidate on a tiny aggregate gain if it materially regresses the colder stress view,
- use `scripts/evaluate_lgbm_submission_proxy.py` as an additional submission-style tail diagnostic, but not as a replacement selector after the warm-only branch improved proxy score and still regressed live public.

## Near-Term Priority Order

Until the current safe branch is exhausted, prefer this sequence:

1. keep `aggregate_overall_score` canonical and treat the colder diagnostics plus watchlists as veto checks, not replacement selectors,
2. use the repaired causal weighted LightGBM residual winner `lgbm_residual_weighted_v2_h131025_leaf31_lr002_mc520_msg020_ff06_bf07_ra25_rl408_warmh13` on top of `ridge_oofnorm_pentaslice_h13_s008_q85_a50_unweighted_v1` as the current top line,
3. use `scripts/predict_lgbm.py` as the only trusted single-policy LGBM submission path; the old ad hoc path was invalidated by the live `0.0000 -> 0.2045` score jump,
4. keep `scripts/evaluate_lgbm_submission_proxy.py` in the loop to measure test-like tail behavior, but treat direct public submissions as the final arbiter whenever they are available,
5. keep the warm-only residual gate closed for public promotion after its `0.1285` live score, even though it improved the pseudo-public tail benchmark,
6. keep the routed-expert cold-gate branch closed after its `0.1990` live score, even though it won on the pseudo-public tail proxy,
7. spend the next overnight budget on the promoted `warmh13` branch itself before reopening broader routing or model-family changes,
8. keep the same-family long-horizon warm-overlay and longwatch specialist branches closed unless a materially different mechanism reopens them,
9. keep direct/stacked LGBM, generic boosting rewrites, deep sequence models, `feature_al`, and broad feature sweeps lower priority until the causal residual LGBM line plateaus.
