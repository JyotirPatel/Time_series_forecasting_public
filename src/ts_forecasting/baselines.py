...
class LinearBlendRegressor:
    ...
    def supports_full_frame_prediction(self) -> bool:
        return all(
            getattr(predictor, "supports_full_frame_prediction", False)
...
class ConditionalLinearBlendRegressor:
    """Blend a default predictor with a slice expert only on matching rows."""

    default_predictor: BatchPredictor
    slice_predictor: BatchPredictor
    blend_rules: list[dict[str, object]]
    parsed_rules_: list[tuple[dict[str, object], float]] | None = None

    @property
    def supports_full_frame_prediction(self) -> bool:
        return bool(
            getattr(self.default_predictor, "supports_full_frame_prediction", False)
            and getattr(self.slice_predictor, "supports_full_frame_prediction", False)
        )
    ...
    def fit(self, train_frame: pd.DataFrame) -> "ConditionalLinearBlendRegressor":
        ...
        known_columns = set(train_frame.columns)
        parsed_rules: list[tuple[dict[str, object], float]] = []
        for rule in self.blend_rules:
            weight = float(rule["weight"])
            if weight < 0.0 or weight > 1.0:
                raise ValueError("blend rule weight must be between 0 and 1")
            if weight == 0.0:
                continue
            conditions = {
                str(column): value
            ...
            unknown_columns = sorted(set(conditions) - known_columns)
            if unknown_columns:
                raise ValueError(
                    f"blend rule columns must exist in the training frame: {unknown_columns}"
                )
            parsed_rules.append((conditions, weight))

        self.parsed_rules_ = parsed_rules
        return self

    def predict_batch(self, batch_frame: pd.DataFrame) -> np.ndarray:
        if self.parsed_rules_ is None:
            raise ValueError("model must be fit before prediction")

        default_predictions = np.asarray(
            self.default_predictor.predict_batch(batch_frame),
            dtype=float,
        )
        if len(self.parsed_rules_) == 0:
            return default_predictions

        slice_predictions = np.asarray(
            self.slice_predictor.predict_batch(batch_frame),
            dtype=float,
        )
        if len(slice_predictions) != len(default_predictions):
            raise ValueError("slice predictor must return one prediction per input row")

        predictions = default_predictions.copy()
        for conditions, weight in self.parsed_rules_:
            blend_mask = np.ones(len(batch_frame), dtype=bool)
            for column, value in conditions.items():
                blend_mask &= batch_frame[column].to_numpy() == value
            if not blend_mask.any():
                continue
            predictions[blend_mask] = (
                (1.0 - weight) * default_predictions[blend_mask]
                + weight * slice_predictions[blend_mask]
            )
        return predictions
