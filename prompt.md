...
Competition data is expected at these local paths when the dataset is mounted on the current host:
...
- If the current host does not yet have `data/train.parquet` and `data/test.parquet`, restore or mount them before running data-dependent training or evaluation commands.
...
- The many-core CPU host should materially improve throughput for CPU-bound preprocessing and LightGBM training, but the runtime now caps auto `num_threads` at `64` and enables `force_col_wise` on many-core hosts, so do not assume linear scaling across all `128` logical CPUs.
