        ...
        for baseline_name in baseline_names:
            ...
            for stress_name, fold_names in STRESS_FOLD_SETS.items():
                if any(_matches_fold_name(fold_name, fold_spec) for fold_spec in fold_names):
                    stress_accumulator[baseline_name]["fold_sets"][stress_name].append(full_summary)
            ...
            for horizon in sorted(scored["horizon"].dropna().unique().tolist()):
                ...
                if len(horizon_scored) == 0:
                    continue
                stress_accumulator[baseline_name]["per_horizon"].setdefault(horizon_key, []).append(
            ...
            for slice_name, definition in STRESS_WATCHLIST_SLICES.items():
                ...
                stress_accumulator[baseline_name]["watchlist_slices"][slice_name].append(
                    summarize_scored_predictions(
                        watchlist_scored,
...
def main() -> None:
    ...
    for fold in folds:
        ...
        fold_stress_input: dict[str, object] = {
            "name": fold["name"],
            "results": {},
        }
        ...
        for name in selected_baselines:
            ...
            fold_stress_input["results"][name] = scored[
                [
                    "code",
                    "sub_category",
                    "horizon",
