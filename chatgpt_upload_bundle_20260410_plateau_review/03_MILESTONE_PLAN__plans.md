...
- add a colder stress-report / veto layer before promoting new warm-heavy variants,
- prioritize clipped or gated stable-group recent-delta variants on horizons `10` and `25`,
- then expand narrow exact warm overlays for `OSJL3A7Y` and `X9BZ68VQ`,
- keep generic boosting, deep sequence models, and broad feature sweeps lower priority until the current leakage-safe stack clearly stalls.
