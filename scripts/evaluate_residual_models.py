...
DEFAULT_OUTPUT_SUFFIX = "oof_norm_incumbent_h13_unweighted"
...
def fit_residual_model(
    ...
    *,
    ...
    model.fit(
        train_frame[RESIDUAL_CATEGORICAL_COLUMNS + numeric_columns],
        train_frame[NORMALIZED_RESIDUAL_TARGET_COLUMN].to_numpy(),
    )
    return model
...
def _sample_frame(
    ...
    *,
    ...
    if len(ordered) <= max_rows:
        return ordered
    return (
        ordered.sample(n=max_rows, random_state=random_state)
...
def _aggregate_config_scores(
    ...
    overall_summary = aggregate_result_summaries(config_fold_rows)
    ...
    *,
    ...
    validation_with_cold_mask = annotate_cold_start_rows(
        full_validation_frame[
            [ID_COLUMN, TARGET_COLUMN, WEIGHT_COLUMN, *cold_start_columns]
        ].copy(),
...
def main() -> None:
    ...
    parser.add_argument(
        "--base-summary-path",
        default=DEFAULT_BASE_SUMMARY_PATH,
    )
    parser.add_argument("--train-path", default="data/train.parquet")
    parser.add_argument("--base-baseline", default=DEFAULT_BASELINE_NAME)
    ...
    cold_start_level = str(base_summary.get("cold_start_level", "code_sub_code"))
    cold_start_columns = COLD_START_LEVELS[cold_start_level]
    first_seen_lookup = build_first_seen_lookup(
        read_parquet_frame(
            args.train_path,
            columns=sorted({TIME_COLUMN, *cold_start_columns}),
        ),
        key_columns=cold_start_columns,
    )
    ...
    stress_accumulator = initialize_stress_accumulator(list(RESIDUAL_CONFIGS))
    ...
    for fold in base_summary["folds"]:
        full_validation_frame, validation_union = _prepare_oof_fold_frames(
            ...
            first_seen_lookup=first_seen_lookup,
            cold_start_columns=cold_start_columns,
        )
        fold_name = str(fold["name"])
    ...
    for fold in base_summary["folds"]:
        ...
        for config_name, config in RESIDUAL_CONFIGS.items():
            ...
            stress_scored = full_validation_frame[
                [
                    ID_COLUMN,
                    CODE_COLUMN,
                    SUB_CATEGORY_COLUMN,
            ...
            full_summary = summarize_scored_predictions(
                stress_scored,
                cold_start_mask=stress_scored["is_cold_start"],
            )
            update_stress_accumulator(
                stress_accumulator,
                fold_name=fold_name,
                baseline_name=config_name,
                scored=stress_scored.rename(
                    columns={
                        CODE_COLUMN: "code",
            ...
            per_config_fold_rows[config_name].append(
                {
                    ...
                    "cold_start_score": full_summary["cold_start_score"],
                    "cold_start_error_sum": full_summary["cold_start_error_sum"],
                    "cold_start_denom_sum": full_summary["cold_start_denom_sum"],
                    "warm_start_score": full_summary["warm_start_score"],
                    "warm_start_error_sum": full_summary["warm_start_error_sum"],
                    "warm_start_denom_sum": full_summary["warm_start_denom_sum"],
                    "cold_start_rows": full_summary["cold_start_rows"],
                    "warm_start_rows": full_summary["warm_start_rows"],
